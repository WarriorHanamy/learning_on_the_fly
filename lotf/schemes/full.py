"""FullScheme: inner-loop dynamics with learned NN residual acceleration (exclusive copy)."""
...
class FullScheme:
    ...
    Exclusive copy of InnerLoopScheme — NOT composable.
    """
    ...
    def _integrate_substep(
        self, state_dict: dict, motor_omega_d: jax.Array, dt: jax.Array, res_acc_mean: jax.Array
    ) -> dict:
        p = state_dict["p"]
        R = state_dict["R"]
        v = state_dict["v"]
        ...
        acc = self._gravity + R @ f_vec / self._params.mass + rotor_acc_residual + res_acc_mean
    ...
    def integrate(
        ...
        # compute NN residual acceleration
        ...
        state_for_res = jnp.array(
            [
                ...
                v[1],
                v[2],
                ap_z * self._params.mass,  # f_d for residual input
                omega_d[0],
                omega_d[1],
                omega_d[2],
            ]
        ...
        def _control_fn(carry, _unused):
            ...
            new_carry = self._integrate_substep(
                carry, motor_omega_cmd, self._dt_low_level, res_acc_mean
            )
