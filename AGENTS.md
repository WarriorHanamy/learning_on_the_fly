...
│   └── __main__.py        # CLI 统一入口点 (uv run train)
│
...
┌─────────────────────────────────────────────────────────────┐
│                        用户接口层                            │
│  CLI (uv run train) | Python API | Jupyter Notebooks      │
└─────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌─────────────────────────────────────────────────────────────┐
...
CSV 数据集 → pandas 加载 → JAX 数组 → 向量化初始化 → 集成训练 → Orbax 检查点
                                                            │
                                                            ▼
CLI (uv run train) → 配置加载 → 环境创建 → BPTT 训练循环 → 策略检查点
```
...
```bash
uv run train --help              # 显示所有命令
uv run train --version           # 显示包版本
uv run train --list-configs      # 列出可用的 YAML 配置
```
